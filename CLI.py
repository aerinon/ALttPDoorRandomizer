...
def parse_arguments(argv, no_defaults=False):
    ...
    # Base ROM
    if (noargs or (not ret.rom) or (ret.rom == "") or (not os.path.isfile(ret.rom))) and (("rom.base" in working_dirs) and (not working_dirs["rom.base"] == "")):
        ret.rom = working_dirs["rom.base"]
    # Enemizer CLI
    if (noargs or (not ret.enemizercli) or (ret.enemizercli == "") or (not os.path.isfile(ret.enemizercli))) and (("enemizer.cli" in working_dirs) and (not working_dirs["enemizer.cli"] == "")):
        ret.enemizercli = working_dirs["enemizer.cli"]
    # Multiworld Names
    if (noargs or (not ret.names) or (ret.names == "")) and (("multi.names" in working_dirs) and (not working_dirs["multi.names"] == "")):
        ret.names = working_dirs["multi.names"]

    if not ret.jsonout and not os.path.isfile(ret.rom) and (not ret.gui and not ret.gui == True):
        input('Could not find valid base rom for patching at expected path %s. Please run with -h to see help for further information. \nPress Enter to exit.' % ret.rom)
        sys.exit(1)
    if ret.sprite and any([sprite is not None and not os.path.isfile(sprite) and not get_sprite_from_name(sprite) for sprite in ret.sprite.values()]):
        if not ret.jsonout:
            input('Could not find link sprite sheet at given location. \nPress Enter to exit.')
            sys.exit(1)
        else:
            raise IOError('Cannot find sprite file at %s' % ret.sprite)
