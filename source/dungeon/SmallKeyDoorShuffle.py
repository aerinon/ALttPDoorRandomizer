...
from KeyDoorShuffle import create_key_counters, prize_relevance, find_big_chest_locations
...
def find_valid_combination(builder, target, start_regions, world, player, drop_keys=True):
    ...
    key_layout = _key_layout_setup(builder, event_starts, player, proposal, start_regions, world, proposed_bk_restrictions)
...
def monte_carlo_algorithm(builder, key_door_pool, key_doors_needed, start_regions, event_starts, custom, world, player):
    ...
    locations = [loc for r in builder.master_sector.regions for loc in r.locations]
...
def exhaustive_key_logic_algorithm(builder, key_door_pool, key_doors_needed, start_regions, event_starts, custom, world, player):
    ...
    while key_doors_needed >= 0:
        ...
        while contradiction_exists and itr < len(sample_list):
            ...
            result = create_exhaustive_placement_rules(key_layout, proposed_bk_restrictions, world, player)
            if not result:
                contradiction_exists = True
            ...
            if not contradiction_exists:
                ...
                bk_required = (builder.bk_door_proposal or any(x in region_set for x in special_bk_regions)
                               or any('- Big Chest' in loc.name for loc in key_layout.item_locations))
                if not is_key_door_layout_satisfiable(key_layout, proposed_bk_restrictions, bk_required):
                    contradiction_exists = True
            if not contradiction_exists:
...
def _key_layout_setup(builder, event_starts, player, proposal, start_regions, world, prop_bk_restrictions=None):
    key_layout = build_key_layout(builder, start_regions, proposal, event_starts, world, player, prop_bk_restrictions)
...
def _sanity_check_placement_rules(key_layout, proposed_bk_restrictions):
    ...
    for rule in key_layout.key_logic.placement_rules:
        if rule.bk_conditional_set is not None:
            ...
            if rule.needed_keys_wo_bk > 0 and available_keys + key_drops == 0:  # no key to place anywhere
                return True
...
def is_key_door_layout_satisfiable(key_layout, bk_restrictions, bk_needed=True, print_proposal=True):
    logger = logging.getLogger('')
    ...
    while looking_for_satisfaction:
        ...
        if not rules_to_check:
            if print_proposal:
                key_layout.print_proposal(logger)
            logger.debug(f'Potential solution found: Small Keys: Anywhere.'
        ...
        if satisfying_key_set is None:
            ...
            if print_proposal:
                key_layout.print_proposal(logger)
            logger.debug(f'Potential solution found:'
...
def determine_big_key_logic(key_layout, world, player):
    ...
    for region in key_layout.sector.regions:
        ...
        free_locations = determine_free_locations(region.locations, world, player)
        ...
        bk_restrictions.update(blind_restricted)
...
def determine_free_locations(location_list, world, player):
    return {loc: None for loc in location_list if not important_location(loc, world, player)
            and not loc.forced_item and loc.name not in dungeon_events}
